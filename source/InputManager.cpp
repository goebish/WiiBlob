...
void InputManager::beginGame(PlayerSide side)
{
	...
	UserConfig config;
	config.loadFile("SD:/data/inputconfig.xml");
...
PlayerInput InputManager::getGameInput(int player)
{
	...
	//if(player==0)
	{
		...
		mInput[player].left = held & WPAD_BUTTON_UP;
		mInput[player].right= held & WPAD_BUTTON_DOWN;
		mInput[player].up = held & WPAD_BUTTON_2;
	}
...
*/
	...
	WPAD_ScanPads(); // todo: temporise
	u32 held = WPAD_ButtonsHeld(0);
	mUp   |= held & WPAD_BUTTON_RIGHT;
	mDown |= held & WPAD_BUTTON_LEFT;
	mLeft |= held & WPAD_BUTTON_UP;
	mRight|= held & WPAD_BUTTON_DOWN;
	mExit |= held & WPAD_BUTTON_HOME;
...
Vector2 InputManager::position()
{
	...
	return Vector2(mMouseX/0.8,mMouseY/0.8);
}
