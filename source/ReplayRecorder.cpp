...
void ReplayRecorder::save(const std::string& filename)
{
	...
	PHYSFS_write(fileHandle, (unsigned char*)validHeader, 1, sizeof(validHeader));
	...
	uint32_t checksum = 0; //crc.checksum();
	PHYSFS_write(fileHandle, (unsigned char*)&checksum, 1, sizeof(checksum));

	PHYSFS_write(fileHandle, (unsigned char*)&mServingPlayer, 1, sizeof(mServingPlayer));
	PHYSFS_write(fileHandle, (unsigned char*)mPlayerNames[LEFT_PLAYER].c_str(), 1, mPlayerNames[LEFT_PLAYER].size()+1);
	PHYSFS_write(fileHandle, (unsigned char*)mPlayerNames[RIGHT_PLAYER].c_str(), 1, mPlayerNames[RIGHT_PLAYER].size()+1);
...
void ReplayRecorder::load(const std::string& filename)
{
	...
	int fileLength = PHYSFS_fileLength(filename.c_str());
	if (fileLength < 8)
	{
	...
	char header[4];
	PHYSFS_read(fileHandle, (unsigned char*)header, 4, 1);
	...
	uint32_t checksum;
	PHYSFS_read(fileHandle, (unsigned char*)&checksum, 1, 4);
	...
	mBufferSize = PHYSFS_read(fileHandle, (unsigned char*)mReplayData, 1, mBufferSize);
	PHYSFS_close(fileHandle);
