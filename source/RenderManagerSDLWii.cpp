...
// goebish todo: find correct blobs, shadows and ball +x decals for 0.8 ratio (see bug #5 on assembla)


#include <iostream>
#include "physfs.h"
#include "RenderManagerSDLWii.h"

SDL_Surface* RenderManagerSDLWii::colorSurface(SDL_Surface *surface, Color color)
{
...
RenderManagerSDLWii::RenderManagerSDLWii()
	: RenderManager()
{
	mBallRotation = 0.0;
...
RenderManager* RenderManager::createRenderManagerSDLWii()
{
	return new RenderManagerSDLWii();
}

void RenderManagerSDLWii::init(int xResolution, int yResolution, bool fullscreen)
{
	Uint32 screenFlags = SDL_HWSURFACE | SDL_HWACCEL | SDL_DOUBLEBUF;
	if (fullscreen)
		screenFlags |= SDL_FULLSCREEN;
	SDL_WM_SetCaption(AppTitle, "");
	SDL_WM_SetIcon(SDL_LoadBMP("SD:/data/Icon.bmp"), NULL);
	...
	SDL_Surface* tempBackground = loadSurface("SD:/data/backgrounds/strand2.bmp");
	mBackground = SDL_DisplayFormat(tempBackground);
	BufferedImage* bgImage = new BufferedImage;
	bgImage->w = mBackground->w; // todo : resize backgrounds images to 640*480 ?
	...
	{
		char filename[64];
		sprintf(filename, "SD:/data/gfx/ball%02d.bmp", i);
	...
	SDL_Surface *tempBallShadow = loadSurface("SD:/data/gfx/schball.bmp");
	...
	{
		char filename[64];
		sprintf(filename, "SD:/data/gfx/blobbym%d.bmp", i);
		...
		sprintf(filename, "SD:/data/gfx/sch1%d.bmp", i);
	...
	{
		char filename[64];
		sprintf(filename, "SD:/data/gfx/font%02d.bmp", i);
	...
	mScroll = loadSurface("SD:/data/gfx/scrollbar.bmp");
}

void RenderManagerSDLWii::deinit()
{
...
void RenderManagerSDLWii::draw()
{
	...
	// Restore the rod
	position.x = (400 - 7)*0.8;
	position.y = 300*0.8;
	SDL_Rect rodPosition;
	rodPosition.x = (400 - 7)*0.8;
	rodPosition.y = 300*0.8;
	rodPosition.w = 14*0.8;
	rodPosition.h = 300*0.8;
...
bool RenderManagerSDLWii::setBackground(const std::string& filename)
{
	try
	{
...
void RenderManagerSDLWii::setBlobColor(int player, Color color)
{
...
void RenderManagerSDLWii::setBall(const Vector2& position, float rotation)
{
	...
	mBallPosition.x = mBallPosition.x*0.8;
	mBallPosition.y = mBallPosition.y*0.8;

	mBallRotation = rotation;
}

void RenderManagerSDLWii::setMouseMarker(float position)
{
	SDL_Rect restore = {
...
void RenderManagerSDLWii::setBlob(int player,
		const Vector2& position, float animationState)
{
	...
	if (player == 0)
	{
		...
		mLeftBlobPosition.x = mLeftBlobPosition.x*0.8;
		mLeftBlobPosition.y = mLeftBlobPosition.y*0.8;
	...
	if (player == 1)
	{
		...
		mRightBlobPosition.x = mRightBlobPosition.x*0.8;
		mRightBlobPosition.y = mRightBlobPosition.y*0.8;
...
void RenderManagerSDLWii::setScore(int leftScore, int rightScore,
...
void RenderManagerSDLWii::setPlayernames(std::string leftName, std::string rightName)
{
	mLeftPlayerName = leftName;
	mRightPlayerName = rightName;
}

void RenderManagerSDLWii::drawText(const std::string& text, Vector2 position, bool highlight)
{
	int length = 0;
	...
	while (index != -1)
	{
		...
		charPosition.x = lround(position.x*0.8) + length;
		charPosition.y = lround(position.y*0.8);
		if (highlight)
...
void RenderManagerSDLWii::drawImage(const std::string& filename, Vector2 position)
{
	mNeedRedraw = true;
	...
	SDL_Rect blitRect = {
		lround(position.x*0.8 - float(imageBuffer->sdlImage->w) / 2.0),
		lround(position.y*0.8 - float(imageBuffer->sdlImage->h) / 2.0),
		lround(position.x*0.8 + float(imageBuffer->sdlImage->w) / 2.0),
		lround(position.y*0.8 + float(imageBuffer->sdlImage->h) / 2.0),
	};
...
void RenderManagerSDLWii::drawOverlay(float opacity, Vector2 pos1, Vector2 pos2, Color col)
{
	redraw(); 
	SDL_Rect ovRect;
	ovRect.x = lround(pos1.x*0.8);
	ovRect.y = lround(pos1.y*0.8);
	ovRect.w = lround(pos2.x*0.8 - pos1.x*0.8);
	ovRect.h = lround(pos2.y*0.8 - pos1.y*0.8);
...
void RenderManagerSDLWii::drawBlob(const Vector2& pos, const Color& col)
{
	SDL_Rect position;
	position.x = lround(pos.x*0.8);
	position.y = lround(pos.y*0.8);
...
void RenderManagerSDLWii::refresh()
{
