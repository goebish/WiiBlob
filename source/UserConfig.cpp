...
bool UserConfig::loadFile(const std::string& filename)
{
	...
	//int fileLength = PHYSFS_fileLength(fileHandle); // goebish
	int fileLength = PHYSFS_fileLength(filename.c_str());
	char* fileBuffer = new char[fileLength + 1];
	PHYSFS_read(fileHandle, (unsigned char*)fileBuffer, 1, fileLength);
...
bool UserConfig::saveFile(const std::string& filename)
{
	...
	PHYSFS_write(fileHandle, (unsigned char*)xmlHeader, 1, sizeof(xmlHeader) - 1);
	fflush(fileHandle);
	for (unsigned int i = 0; i < mVars.size(); ++i)
	{
		...
		PHYSFS_write (fileHandle, (unsigned char*)writeBuffer, 1, charsWritten);
		fflush(fileHandle);
	}

	PHYSFS_write(fileHandle, (unsigned char*)xmlFooter, 1, sizeof(xmlFooter) - 1);
	fflush(fileHandle);
	PHYSFS_close(fileHandle);
	return true;
}
