...
OptionState::OptionState()
{
	...
	//char** filenames = PHYSFS_enumerateFiles("SD:/data/scripts");
...
OptionState::~OptionState()
{
	if (mSaveConfig)
	{
		...
		mOptionConfig.saveFile("SD:/data/config.xml");
	}
}
...
GraphicOptionsState::~GraphicOptionsState()
{
	if (mSaveConfig)
	{
			...
			(mOptionConfig.getString("device") != mRenderer))
		{
			...
			//else // force SDL ( goebish )
				RenderManager::createRenderManagerSDLWii()->init(640, 480, mFullscreen); // todo > change to 640*480 ?
...
void GraphicOptionsState::step()
{
	...
	if (imgui.doButton(GEN_ID, Vector2(34.0, 110.0), "Window Mode"))
		mFullscreen = false;
	if (mFullscreen)
		imgui.doImage(GEN_ID, Vector2(18.0, 92.0), "SD:/data/gfx/pfeil_rechts.bmp");
	else
		imgui.doImage(GEN_ID, Vector2(18.0, 122.0), "SD:/data/gfx/pfeil_rechts.bmp");
	...
	if (imgui.doButton(GEN_ID, Vector2(444.0, 110.0), "SDL"))
		mRenderer = "SDL";
	if (mRenderer == "OpenGL")
		imgui.doImage(GEN_ID, Vector2(428.0, 92.0), "SD:/data/gfx/pfeil_rechts.bmp");
	else
		imgui.doImage(GEN_ID, Vector2(428.0, 122.0), "SD:/data/gfx/pfeil_rechts.bmp");
	...
	if (imgui.doButton(GEN_ID, Vector2(220.0, 380), "no"))
		mLeftMorphing = false;
	if (mLeftMorphing)
		imgui.doImage(GEN_ID, Vector2(54.0, 392.0), "SD:/data/gfx/pfeil_rechts.bmp");
	else
		imgui.doImage(GEN_ID, Vector2(204.0, 392.0), "SD:/data/gfx/pfeil_rechts.bmp");
	//draw left blob:
	{
	...
	if (mRightMorphing)
		imgui.doImage(GEN_ID, Vector2(454.0, 392.0), "SD:/data/gfx/pfeil_rechts.bmp");
	else
		imgui.doImage(GEN_ID, Vector2(604.0, 392.0), "SD:/data/gfx/pfeil_rechts.bmp");
	//draw right blob:
	{
		float time = float(SDL_GetTicks()) / 1000.0;
...
InputOptionsState::InputOptionsState()
{
	...
	mSaveConfig = false;
	mSetKeyboard = 0;
	mOptionConfig.loadFile("SD:/data/inputconfig.xml");
	//left data:
...
InputOptionsState::~InputOptionsState()
{
	if (mSaveConfig)
	{
		...
		mOptionConfig.saveFile("SD:/data/inputconfig.xml");
	}
}
...
MiscOptionsState::~MiscOptionsState()
{
	...
	RenderManager::getSingleton().setBackground(std::string("SD:/data/backgrounds/") + mOptionConfig.getString("background"));
}
...
void MiscOptionsState::step()
{
	...
	if (tmp != mBackground)
	{
		mBackground = tmp;
		if (mBackground > -1)
			RenderManager::getSingleton().setBackground(std::string("SD:/data/backgrounds/") + mBackgrounds[mBackground]);
	}
	...
	if (imgui.doScrollbar(GEN_ID, Vector2(484.0, 50.0), mVolume))
	{
		...
		SoundManager::getSingleton().playSound("SD:/data/sounds/bums.wav", 1.0);
	}
	...
	if (mMute)
	{
		imgui.doImage(GEN_ID, Vector2(513.0, 92.0), "SD:/data/gfx/pfeil_rechts.bmp");
	}
	...
	if (mShowFPS)
	{
		imgui.doImage(GEN_ID, Vector2(466.0, 132.0), "SD:/data/gfx/pfeil_rechts.bmp");
	}
	...
	if (mShowBlood)
	{
		imgui.doImage(GEN_ID, Vector2(466.0 ,172.0), "SD:/data/gfx/pfeil_rechts.bmp");
	}
	...
	if (imgui.doButton(GEN_ID, Vector2(630.0, 240.0), "Right"))
		mNetworkSide = 1;
	if (mNetworkSide == 0)
		imgui.doImage(GEN_ID, Vector2(432.0, 252.0), "SD:/data/gfx/pfeil_rechts.bmp");
	else
		imgui.doImage(GEN_ID, Vector2(612.0, 252.0), "SD:/data/gfx/pfeil_rechts.bmp");
