...
void MainMenuState::step()
{
	...
	imgui.doImage(GEN_ID, Vector2(250.0, 210.0), "SD:/data/gfx/titel.bmp");
	/*if (imgui.doButton(GEN_ID, Vector2(484, 370.0), "network game")) // todo goebish : reactivate network
	{
	...
	if (imgui.doButton(GEN_ID, Vector2(484.0, 400.0), "start"))
	{
		...
		{
			//FILE* file = fopen("SD:/data/lualog.txt", "wb+"); // was "wb" goebish
...
void CreditsState::step()
{
	...
	imgui.doImage(GEN_ID, Vector2(400.0, 300.0), "background");
	imgui.doOverlay(GEN_ID, Vector2(0.0, 0.0), Vector2(800.0, 600.0));
	imgui.doText(GEN_ID, Vector2(40.0, mYPosition), "original programmers:");
	imgui.doText(GEN_ID, Vector2(40.0, mYPosition+30), "Jonathan Sieber");
	imgui.doText(GEN_ID, Vector2(40.0, mYPosition+60), "Daniel Knobe");
	imgui.doText(GEN_ID, Vector2(40.0, mYPosition+95), "Sven Rech");
	imgui.doText(GEN_ID, Vector2(40.0, mYPosition+125), " ");
	imgui.doText(GEN_ID, Vector2(40.0, mYPosition+160), "Wii port:");
	imgui.doText(GEN_ID, Vector2(40.0, mYPosition+190), "Goebish");

	imgui.doText(GEN_ID, Vector2(40.0, mYPosition+230), " ");
	imgui.doText(GEN_ID, Vector2(40.0, mYPosition+260), "graphics:");
	imgui.doText(GEN_ID, Vector2(40.0, mYPosition+290), "Silvio Mummert");
	imgui.doText(GEN_ID, Vector2(40.0, mYPosition+325), "Richard Bertrand");
	imgui.doText(GEN_ID, Vector2(40.0, mYPosition+355), " ");
	...
	imgui.doText(GEN_ID, Vector2(40.0, mYPosition+460), " ");
	...
	mRightPlayer(RIGHT_PLAYER)
{
	...
	mReplayRecorder = 0;
	mSelectedReplay = 0;
	char** filenames = PHYSFS_enumerateFiles("SD:/data/replays");
...
void ReplayMenuState::loadCurrentReplay()
{
	...
	try
	{
		mReplayRecorder->load(std::string("SD:/data/replays/" + mReplayFiles[mSelectedReplay] + ".bvr"));
		mReplaying = true;
		...
		SoundManager::getSingleton().playSound(
				"SD:/data/sounds/pfiff.wav", ROUND_START_SOUND_VOLUME);
	}
...
void ReplayMenuState::step()
{
	...
	if (mReplaying)
	{
		...
		if (side != NO_PLAYER)
		{
			...
			imgui.doImage(GEN_ID, Vector2(200, 250), "SD:/data/gfx/pokal.bmp");
	...
	else
	{
		...
		if (imgui.doButton(GEN_ID, Vector2(644.0, 60.0), "delete"))
		{
			...
			if (PHYSFS_delete(std::string("SD:/data/replays/" + mReplayFiles[mSelectedReplay] + ".bvr").c_str()))
			{
