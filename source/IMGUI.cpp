...
void IMGUI::end()
{
	...
	while (!mQueue->empty())
	{
		...
		switch (obj.type)
		{
			...
			case SCROLLBAR:
				...
				rmanager.drawImage("SD:/data/gfx/scrollbar.bmp",obj.pos1 + Vector2(obj.pos2.x * 200.0 + 5 , 13));
				break;
	...
	if (mDrawCursor)
	{
		rmanager.drawImage("SD:/data/gfx/cursor.bmp", 
...
bool IMGUI::doSelectbox(int id, const Vector2& pos1, const Vector2& pos2, const std::vector<std::string>& entries, int& selected)
{
	...
	doImage(GEN_ID, Vector2(pos2.x-15, pos1.y+15), "SD:/data/gfx/pfeil_oben.bmp");
	doImage(GEN_ID, Vector2(pos2.x-15, pos2.y-15), "SD:/data/gfx/pfeil_unten.bmp");
